...
const patchScore = async (req: Request<{ id: string }, {}, { token: string, result: string, goals: string[] | string }, {}>, res: Response) => {
    try {
        ...
        const newGame: IGame | null = await Game.findByIdAndUpdate(req.params.id, { result, goals: newGoals }, { new: true })
